...
本文说明如何安装并测试 VistrTavern `v0.6.0-alpha`。
...
打开 `可选设定` -> `创作背景卡`，描述本地故事上下文：

- `世界观`：规则、类型或设定。
- `剧情背景`：房间里已经发生了什么。
- `可客串角色槽位`：人类访客可以短暂客串哪些角色。
- `AI 世界维护规则`：AI 负责维持什么，比如连续性、NPC 反应和世界后果。

这是本地创作者元数据，用于导出和脑暴；它是背景卡，不是在线房间。
...
VistrTavern 会把选中的角色标记为真人控制状态。在这个窗口内，VistrTavern 可以把你输入的一句话作为该角色消息插入当前 SillyTavern 聊天，同时把同一句记录为真人接管造成的异常发言。

面板会显示 `正在接管 {角色}` 和接下来的操作步骤。如果发现选错了角色，可以点击 `结束` 后重新选择并开始接管。

### 9. 发送接管发言

在 `接管发言` 里输入真正的客串台词，然后点击 `发送为角色并记录`。

VistrTavern 会优先通过 SillyTavern 扩展 API 调用内置 `/sendas` 命令。如果该 API 不可用，则 fallback 到 SillyTavern 暴露的 chat 渲染接口直接插入消息。

发送前可以选择 `乱入类型`：

- `角色接管`：普通的真人接管角色发言。
- `异常发言`：刻意制造不合 AI 群像节奏的错位感。
- `记忆断片`：后续可以被角色理解为失控、迟疑或断片。
- `外部意志`：暗示外力、诅咒、舞台规则或现实不稳定。
- `剧情钩子`：目的是打开下一步后果或转折。
- `关系破坏`：目的是破坏、压迫或试探一段关系。
- `线索污染`：改变证词、证据、动机或推理链。
- `世界规则裂缝`：暗示世界规则、系统或边界发生裂缝。
...
同时会创建一个扰动事件。SillyTavern 里可见的消息会表现为角色消息，而不是用户消息。只有在你明确不想把内容写入当前聊天时，才使用 `仅记录`。
