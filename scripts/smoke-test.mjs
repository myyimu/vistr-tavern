...
import { AwarenessScope, BrainstormKind, BranchType, Controller, HandoffAwareness, IntrusionKind, ScenarioPreset, Visibility, createEmptyMemory } from '../data/schema.js';
...
const humanMessage = memory.recordMessage({
  ...
  visibility: Visibility.ANONYMOUS,
  intrusionKind: IntrusionKind.CLUE_CONTAMINATION,
...
assert.match(markdown, /乱入类型/);
assert.match(markdown, /线索污染/);
...
assert.match(organizedMaterial, /线索污染/);
...
assert.match(englishOrganizedMaterial, /Clue contamination/);
...
const zhMemory = new NarrativeMemory(createEmptyMemory(new Date(now)));
const zhSceneManager = new SceneManager(zhMemory.memory);
const zhEngine = new IntrusionEngine({ now: () => now });
zhMemory.memory.session.promptLanguage = 'zh-CN';
zhMemory.syncCharacters([{ name: '艾琳' }, { name: '宰相' }]);
const zhScene = zhSceneManager.setScene({ name: '王都宴会', mood: '压迫', tension: 82 });
zhEngine.addEventListener('intrusion:start', (event) => zhMemory.recordIntrusionStarted(event.detail));
zhEngine.addEventListener('intrusion:end', (event) => zhMemory.recordIntrusionEnded(event.detail));
const zhIntrusion = zhEngine.startIntrusion({
  characterId: '艾琳',
  characterName: '艾琳',
  durationMs: 60_000,
  visibility: Visibility.ANONYMOUS,
...
zhMemory.recordMessage({
  characterId: '艾琳',
  speakerName: '艾琳',
  controller: Controller.HUMAN,
  visibility: Visibility.ANONYMOUS,
  intrusionKind: IntrusionKind.PLOT_HOOK,
  content: '你们真的相信国王还活着吗？',
  scene: zhScene,
